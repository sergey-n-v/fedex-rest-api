    ...
    public function populateFromStdClass(\stdClass $stdClass)
    {
        try {
            ...
            foreach ($setterMethods as $reflectionMethod) {
                ...
                $stdPropertyName = lcfirst(str_replace('set', '', $methodName));
                if (!property_exists($stdClass, $stdPropertyName)) {
                    $stdPropertyName = Str::unCamelCase($stdPropertyName);
                }
                $parameterValue = null;
...
//                        var_dump($arrayType, $reflectionParameter, Reflection::isClassNameSimpleType($arrayType));
...
//                                    var_dump($class);
            ...
            $this->values = json_decode(json_encode($stdClass),1);
        }
    }
}
